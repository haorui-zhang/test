...
def beam_search_decode(model, src, src_mask, max_len, start_symbol, beam_size, end_idx):
    ...
    for i in range(max_len - 1):
        ...
        temp_test = False 
        score_unsqueeze = scores.unsqueeze(1)
        score = score_unsqueeze.expand_as(prob) + prob 
        val, idx = torch.topk(score.view(-1), beam_size, sorted=False) 

        # sample test
        if i == 0 and temp_test == True:
            print('Probability:', prob)
            print('New_score', score)
            print('Top_validation:',val)
        
        # convert top_idx back to 2D
        res = []
        for id in idx:
            rec_pos = id // vocab_size
            now_pos = id % vocab_size
            if ys[rec_pos, -1].item() != end_idx:
                rec_posit = torch.cat([ys[rec_pos]])
                now_posit = torch.tensor([now_pos]).cuda()
                res.append(rec_posit, now_posit)
            else:
                rec_posit = torch.cat([ys[rec_pos]])
                now_posit = torch.tensor([end_idx]).cuda()
                res.append(rec_posit, now_posit)

        # update scores and ys
        ys = torch.stack(res) # size (beam_size, i+1)
        scores = val
